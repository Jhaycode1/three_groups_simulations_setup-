...
# !!! SAVE NAMES !!! 
save_location <- "./test_run_results/"
save_name <- paste0("test_run_1", my_seed1, "_", Sys.Date()) #This is used to create a filename, done by concatenating the string "test_run_1" with the seed number and the date of the run. This is used to save the results of the simulation in a unique file name.
...
num_individuals_GWAS <- 1000 # The total number of individual or people's GWAS genetic data needed that we want to simulate. The number is configurable.
# Notice that the number of individuals for RNA-seq and GWAS can be different. This is because the data is collected from different sources and the number of individuals may not be the same. That is because the RNA-seq data is always lower in noise and efficient and takes place in small number of people comapred to the GWAS data that the differences between individual is low, so much number is needed for high statistical power.

num_genes            <- 30 #This set te total number of genes that we want to simulate in total. 
num_beneficial       <- 10 # set the number of genes that are beneficial or have a positive effect on the outcome. This is used to simulate the data and to evaluate the performance of the methods.
...
#############################
# Three_groups
models_for_mcmc <- c("combined", "RNA_only", "GWAS_only") #This is used to specify the models that we want to run in the MCMC simulation. The models are "combined", "RNA_only", and "GWAS_only". The combined model uses both RNA-seq and GWAS data for joint inference analysis, while the RNA_only model uses only RNA-seq data, and the GWAS_only model uses only GWAS data. This is used to evaluate the performance of the methods.
priors_for_mcmc <- c("piMOM", "local")
niter           <- 100 # This set the number of iteration that want you want the MCMC sampling to run amd simnulate the data. The number of iterations is configurable and can be set to any number. The higher the number of iterations, the more accurate the results will be, but it will also take longer to run. The lower the number of iterations, the less accurate the results will be, but it will also take less time to run.
nburnin         <- 25 # This set the number of burn-in iterations that you want the MCMC sampling to run and simulate the data. The number of burn-in iterations is configurable and can be set to any number. The higher the number of burn-in iterations, the more accurate the results will be, but it will also take longer to run. The lower the number of burn-in iterations, the less accurate the results will be, but it will also take less time to run.
thin            <- 2 # This set the thinning interval that you want the MCMC sampling to run and simulate the data. The thinning interval is configurable and can be set to any number. The higher the thinning interval, the more accurate the results will be, but it will also take longer to run. The lower the thinning interval, the less accurate the results will be, but it will also take less time to run, which is the number of steps or the result that want the model simulation to keep. Eg, here on 100 simulation, the model will keep 2 results, one at 51 and the other at 101, and discard the rest. This is used to reduce the autocorrelation in the MCMC samples and to improve the mixing of the chains. The thinning interval is set to 50, which means that every 50th sample will be kept and the rest will be discarded. This is used to reduce the autocorrelation in the MCMC samples and to improve the mixing of the chains.
